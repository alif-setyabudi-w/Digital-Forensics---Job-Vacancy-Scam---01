...
-Pemeriksaan awal dilakukan dengan ls -lh untuk melihat ukuran file.
-Integritas file dijaga dengan menghasilkan nilai hash SHA256 menggunakan sha256sum.
...
-Menggunakan perintah strings yang digabungkan dengan grep untuk mengekstrak dan mencari URL atau tautan tersembunyi di dalam file PDF.
...
-Melakukan pencarian spesifik menggunakan grep pada teks yang diekstrak untuk menemukan kata kunci indikasi penipuan (Red Flags) seperti "SMS", "Transfer", "Pembayaran", atau nomor telepon.
...
-Mendapatkan informasi struktural PDF menggunakan pdfinfo (versi PDF, ukuran halaman, jumlah halaman).
-Mengecek adanya file yang disematkan (embedded files) dengan mencari string spesifik.
...
-Memastikan tidak ada lampiran tersembunyi menggunakan pdfdetach.
...
-Mengekstrak metadata dari salah satu gambar yang dihasilkan.
-Melakukan verifikasi hash SHA256 pada seluruh gambar yang diekstrak (.jpg). 
...
-Memverifikasi tipe file sebenarnya dari objek yang diekstrak menggunakan perintah file.
...
-Menggunakan exiftool untuk mengekspor metadata dari dua gambar yang berbeda ke dalam file teks (metadata_001.txt dan metadata_002.txt).
...
-Menerapkan looping di shell dengan xxd untuk membaca beberapa byte pertama dari seluruh file .jpg secara massal.
...
-Menggunakan jpeginfo untuk memverifikasi integritas struktural dari gambar yang diekstrak (memastikan tidak ada file yang corrupt).
